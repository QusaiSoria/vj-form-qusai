...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nما فيك تطلب هون.اشترك في بوت  <a href='https://t.me/MosalsalatSoriaKadema101'>المسلسلات السورية</a> او من هنا 👇</b>",
        reply_markup=InlineKeyboardMarkup(
        [[InlineKeyboardButton("📝 اشترك من هنا", url="https://t.me/+5tUz6yhzgt41OTQ0")]]
    )  )
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("الصفحة", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("التالي ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("الصفحة", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("التالي ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ السابق", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    try:
        if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"⚠️ هلا{query.from_user.first_name},\nهدا مش طلب,\nاطلب لنفسك 😁...",
                show_alert=True,
            )
    except:
        pass
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("الصفحة", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="التالي� ➪",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("الصفحة", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="التالي� ➪",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
        ...
        btn.append(
            [InlineKeyboardButton(text="خلصوا الصفحات 😁",callback_data="pages")]
        )
    if lang != "homepage":
        req = query.from_user.id
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if not free_trial_status:            
            ...
            new_text = "**حصلت على تجربة مجانية لمدة 5 دقائق 😄**"        
            await query.message.edit_text(text=new_text)
            return
        else:
            new_text= "** لقد قمت بالتجربة من قبل 😁. يمكنك الإشتراك من هنا 👈 /plan**"
            await query.message.edit_text(text=new_text)
            return
        ...
        btn.append(
            [InlineKeyboardButton("⚠️إغلاق / حذف⚠️", callback_data="close_data")]
        )
        reply_markup = InlineKeyboardMarkup(btn)
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("السابق", callback_data="backcb")]
        ])
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await query.answer(f"هلا {query.from_user.first_name}, هذا ليس لطلبك ,اطلب لنفسك 😁 !", show_alert=True)
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    return await reply_msg.edit_text(f"**⚠️ لم أجد اي شئ لطلبك  - {name}**\n**تأكد من طريقة طلبك او انتظر الأدمن ليقوم بتوفير طلبك.**")
        else:
            return
    else:
